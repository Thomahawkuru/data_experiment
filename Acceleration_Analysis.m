...
if calculated == false
    for P = first : last
        ...
        for T = 1:Scene1(P).Trails
            [Scene1(P).Input(T).Acceleration, Scene1(P).Input(T).MaxAcceleration] = ...
                InputAcceleration(Scene1(P).Input(T).Velocity, Scene1(P).Input(T).Time, plotting);
        end
        for T = 1:Scene2(P).Trails
            [Scene2(P).Input(T).Acceleration, Scene2(P).Input(T).MaxAcceleration] = ...
                InputAcceleration(Scene2(P).Input(T).Velocity, Scene2(P).Input(T).Time, plotting);             
        end
    end
    clear Acceleration
...
%% Plotting
figure(8); clf(8); 
Plotting(Acceleration_1, Acceleration_2, [Scene1.avgAccelerationMax], [Scene2.avgAccelerationMax], conditions, 'Max Acceleration', '[m/s]');
