...
if calculated == false
    for P = first : last
        ...
        for T = 1:Scene1(P).Trails
            [Scene1(P).Input(T).Velocity, Scene1(P).Input(T).MaxVelocity] = ...
                InputVelocity(Scene1(P).Input(T).Path, Scene1(P).Input(T).Time, plotting);
        end
        for T = 1:Scene2(P).Trails
            [Scene2(P).Input(T).Velocity, Scene2(P).Input(T).MaxVelocity] = ...
                InputVelocity(Scene2(P).Input(T).Path, Scene2(P).Input(T).Time, plotting);             
        end
    end
    clear Velocity
    ...
    for i = first:last
        Scene1(i).avgVelocityMax = mean([Scene1(i).Input.MaxVelocity], 'omitnan');
        Scene2(i).avgVelocityMax = mean([Scene2(i).Input.MaxVelocity], 'omitnan');
        Velocity_1 = cat(1,Velocity_1,Scene1(i).Input.MaxVelocity);
        Velocity_2 = cat(1,Velocity_2,Scene2(i).Input.MaxVelocity);
    end
end

%%Plotting
figure(7); clf(7); 
Plotting(Velocity_1, Velocity_2, [Scene1.avgVelocityMax], [Scene2.avgVelocityMax], conditions, 'Max Velocity', '[m/s]');
