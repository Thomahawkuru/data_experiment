...
if calculated == false
    ...
    for P = first : last
        ...
        for T = 1:Scene1(P).Trails
            [Scene1(P).Output(T).Jerk, Scene1(P).Input(T).MaxJerk] = ...
                InputJerk(Scene1(P).Output(T).Acceleration, Scene1(P).Input(T).Time, plotting);
        end
        for T = 1:Scene2(P).Trails
            [Scene2(P).Output(T).Jerk, Scene2(P).Input(T).MaxJerk] = ...
                InputJerk(Scene2(P).Output(T).Acceleration, Scene2(P).Input(T).Time, plotting);             
        end
    end
    clear Jerk
...
%% Plotting
figure(22); clf(22); 
Plotting(Jerk_1, Jerk_2, [Scene1.avgJerkMax], [Scene2.avgJerkMax], conditions, 'Max Robot Jerk', '[m/s^3]');
